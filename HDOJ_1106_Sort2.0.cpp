...
void each_case(string &str){
    ...
    regex SAP("[0-46-9]+");
    regex SUB_JUG("[1-46-9]\\d*");
    vector<string> str_ctn;
    for(sregex_iterator it(str.begin(), str.end(),SAP), end_it; it != end_it; ++it){
        string sub_str = it->str();
        smatch match_result;
        if(regex_search(sub_str,match_result,SUB_JUG)==true) str_ctn.push_back(match_result.str());
        else str_ctn.push_back("0");
    }
...
//Summing Up
//Compared to HDOJ_1106_Sort1.0.cpp, simplify application of RegEX.
